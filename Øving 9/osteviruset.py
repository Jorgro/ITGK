...
harmful = ['B15-1', 'B15-2', 'B15-3', 'B15-4', 'A234-2', 'A234-3', 'A234-4', 'A235-1', 'A235-2', 'A235-3', 'C31-1', 'C31-2', 'A234-5']
potential = set()


for space in harmful:
    for key in cheeses:
        if space in cheeses[key]:
            potential.add(key)
print(potential)
for key in cheeses:
    for i in range(len(cheeses[key])):
        if key not in potential:
            print(f'{cheeses[key][i].ljust(10)} {key}')
